...
const course = await courses.get.req({ id: props.id })
const teacher = await teachers.info.req({
    id: (await courses.get.req({ id: props.id })).course.owner_user_id
})
</script>
<template>
    <div @click="$router.push('/courses/' + $props.id)"
        style=" position: relative; border-radius: 10px; background-color: #eee; width: 100%; overflow: hidden; display: flex; flex-direction: row; align-items: flex-start; justify-content: flex-start; padding: 20px; box-sizing: border-box; gap: 20px; cursor: pointer; text-align: left; font-size: 36px; color: #000; font-family: Roboto;"
        id="frameContainer3">
        <div style="flex: 1;display: flex;flex-direction: column;align-items: flex-start;justify-content: flex-start;">
            ...
            <div style="position: relative; font-size: 16px">
                автор: <span style="text-decoration: underline">{{ (teacher?.info.name) }}</span>
            </div>
        </div>
        <div
